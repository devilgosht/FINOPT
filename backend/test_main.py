...
def test_calculation_different_rates_and_tenures():
    # 50,000 at 8.0% p.a. for 6 months, compounded quarterly (n=4, t=0.5, periods=2)
    # A = 50000 * (1 + 0.08/4)**2 = 50000 * (1.02)**2 = 52020.0
    calc = calculate_compound_interest(
        principal=50000.0,
...
def test_calculation_monthly_compounding():
    # 10,000 at 6.0% p.a. for 12 months, compounded monthly (n=12)
    # A = 10000 * (1 + 0.06/12)**12 = 10000 * (1.005)**12 = 10616.78
    calc = calculate_compound_interest(
        principal=10000.0,
...
def test_product_eligibility_by_amount():
    ...
    pairs_low = find_eligible_tenure_pairs(all_prods, amount=5000, target_months=12)
    zenith_matched = any(p.institution.startswith("Zenith") for p, _ in pairs_low)
    assert not zenith_matched

    # When amount is 1,00,000, Zenith SHOULD be matched.
    pairs_high = find_eligible_tenure_pairs(all_prods, amount=100000, target_months=12)
    zenith_matched_high = any(p.institution.startswith("Zenith") for p, _ in pairs_high)
...
def test_product_eligibility_by_tenure_window():
    ...
    pairs = find_eligible_tenure_pairs(all_prods, amount=50000, target_months=12, tenure_window=6)
...
def test_ranking_deterministic_and_structured_reasons():
    all_prods = get_all_products()
    pairs = find_eligible_tenure_pairs(all_prods, amount=100000, target_months=12)
...
def test_longer_tenure_optimization_for_return_goal():
    ...
    best_direct = next(r for r in ranked if r.tenure_months == 12)
...
def test_api_recommend_invalid_amount():
    ...
    assert "Investment amount must be greater than 0" in response.json()["detail"]
...
def test_api_recommend_invalid_tenure():
    ...
    response = client.post("/api/recommend", json=payload)
    assert response.status_code == 400
    assert "Time horizon must be at least 1 month" in response.json()["detail"]
...
@pytest.mark.parametrize("goal", ["safe_deposit", "better_return", "need_liquidity"])
...
@pytest.mark.parametrize("liquidity_pref", ["high", "balanced", "can_lock_longer"])
...
def test_e2e_recommendations_vary_based_on_user_intent():
    ...
    res_yield = client.post("/api/recommend", json=payload_yield).json()
    ...
    res_liquidity = client.post("/api/recommend", json=payload_liquidity).json()
    ...
    # Return goal should optimize towards longer tenure; liquidity goal towards shorter or high-liquidity
